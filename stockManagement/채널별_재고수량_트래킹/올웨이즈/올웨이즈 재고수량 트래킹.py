...
ws.cell(3, 13).value = '상품정보'
ws.cell(3, 14).value = '상품명'
ws.cell(3, 15).value = '컬러'
ws.cell(3, 16).value = '사이즈'
ws.cell(3, 17).value = '주문정보 정제'
ws.cell(3, 18).value = '재고(데이터파일 기준)'

ws.cell(3, 13).alignment = fillAlignment
ws.cell(3, 14).alignment = fillAlignment
ws.cell(3, 15).alignment = fillAlignment
ws.cell(3, 16).alignment = fillAlignment
ws.cell(3, 17).alignment = fillAlignment
ws.cell(3, 18).alignment = fillAlignment

ws.cell(3, 13).font = fillFont
ws.cell(3, 14).font = fillFont
ws.cell(3, 15).font = fillFont
ws.cell(3, 16).font = fillFont
ws.cell(3, 17).font = fillFont
ws.cell(3, 18).font = fillFont

ws.cell(3, 13).fill = fillData
ws.cell(3, 14).fill = fillData
ws.cell(3, 15).fill = fillData
ws.cell(3, 16).fill = fillData
ws.cell(3, 17).fill = fillData
ws.cell(3, 18).fill = fillData
    ...
    if stockList[ws.cell(i, 17).value] == 0:
      if ws.cell(row=i, column=4).value == "판매중":
        ...
        if ws.cell(i, 17).value in soldoutPrdCSList:
          stockErrList.append("○ {} / 상태 : {} / 재고수량 : {} / 데이터파일 기준 재고 : 0".format(ws.cell(i, 17).value, ws.cell(i, 4).value, ws.cell(i, 12).value))
        else:
          stockErrAutoList.append("※ 판매량차감 자동품절 상품(CS팀에서 품절로 전달되지 않은 상품) ※\n○ {} / 상태 : {} / 재고수량 : {} / 데이터파일 기준 재고 : 0".format(ws.cell(i, 17).value,ws.cell(i, 4).value, ws.cell(i, 12).value))
        for colNum in range(1, 19):
          ws.cell(row=i, column=colNum).fill = fillData2
            
    if stockList[ws.cell(i, 17).value] != 0:
      if int(ws.cell(row=i, column=12).value) <= 3:
        if ws.cell(row=i, column=4).value == "판매중":
          if stockList[ws.cell(i, 17).value] > int(ws.cell(row=i, column=12).value):
            impendingPrdList.append("○ {} / 상태 : {} / 재고수량 : {} / 데이터파일 기준 재고 : {}".format(ws.cell(i, 17).value, ws.cell(i, 4).value, ws.cell(i, 12).value, stockList[ws.cell(i, 17).value]))
          
      if ws.cell(row=i, column=4).value == "판매중":
        if prdDetailInfoProduct in excProducts:
          excProductsCheckList.append("○ {} / 상태 : {} / 재고수량 : {}".format(ws.cell(i, 17).value, ws.cell(i, 4).value, ws.cell(i, 12).value))
...
if len(stockErrList) > 0 or len(stockErrAutoList) > 0:
  f = open("(올웨이즈)) 품절상품 중 판매세팅된 상품 정보.txt", "w")
  f.write("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n\n")
  f.write("(올웨이즈) 품절상품 중 판매세팅된 상품 정보\n\n")
...
if len(impendingPrdList) > 0:
  f = open("(올웨이즈) 재고 보충 필요 상품 정보(품절 혹은 품절임박).txt", "w")
  f.write("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n\n")
  f.write("(올웨이즈) 재고 보충 필요 상품 정보(품절 혹은 품절임박)\n\n")
  for i in impendingPrdList:
    f.write("{}\n\n".format(i))
  f.close()
  
if len(excProductsCheckList) > 0:
  f = open("(올웨이즈) 판매제외 상품 포함 체크.txt", "w")
  f.write("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n\n")
  f.write("(올웨이즈) 판매제외 상품 포함 체크\n\n")
  for i in excProductsCheckList:
    f.write("{}\n\n".format(i))
  f.close()
  
if len(matchingErrList) > 0:
  f = open("(올웨이즈) 상품정보 매칭 오류건.txt", "w")
  f.write("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n\n")
  f.write("(올웨이즈) 상품정보 매칭 오류건\n\n")
