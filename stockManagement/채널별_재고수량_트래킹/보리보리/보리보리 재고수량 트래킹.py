...
for i in range(first_row, last_row):
  try:
    ...
    ws.cell(i, 18).value = ws.cell(i, 18).value.replace(" ", "")
    ...
    if stockList[ws.cell(i, 22).value] == 0:
      if ws.cell(row=i, column=16).value == "Y":
        if int(ws.cell(row=i, column=12).value) != 0 or int(ws.cell(row=i, column=13).value) != 0:
          ...
          if ws.cell(i, 22).value in soldoutPrdCSList:
            ...
            stockErrAutoList.append("※ 판매량차감 자동품절 상품(CS팀에서 품절로 전달되지 않은 상품) ※\n○ {} / 상품코드 : {} / 실재고 : {} / 임시재고 : {} / 사용여부 : {} / 데이터파일 기준 재고 : 0".format(ws.cell(i, 22).value, ws.cell(i, 7).value, ws.cell(i, 12).value, ws.cell(i, 13).value, ws.cell(i, 16).value))
    ...
    if ws.cell(row=i, column=16).value == "Y":
      if int(ws.cell(row=i, column=12).value) != 0 or int(ws.cell(row=i, column=13).value) != 0:
        ...
        # 판매세팅 상품정보 추가
        if ws.cell(i, 19).value not in settingInfo:
          settingInfo[ws.cell(i, 19).value] = [channelName]
        else:
          if channelName not in settingInfo[ws.cell(i, 19).value]:
            settingInfo[ws.cell(i, 19).value].append(channelName)
...
if len(impendingPrdList) > 0:
  f = open("(보리보리) 재고 보충 필요 상품 정보(품절 혹은 품절임박).txt", "w")
  f.write("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n\n")
...
if len(excProductsCheckList) > 0:
  f = open("(보리보리) 판매제외 상품 포함 체크.txt", "w")
  f.write("ㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ\n\n")
  f.write("(보리보리) 판매제외 상품 포함 체크\n\n")
