...
wb2Sheet.column_dimensions['X'].width = 12.5
...
for i in range(wb2FirstCell, wb2LastCell):
  ...
  try:
    ...
    # TODO 기준 파일에 맞춰 정보 수정 필요
    image_path = '.\\data\\images\\' + str(productCode[wb2Sheet.cell(row=i, column=4).value]) + '.jpg'
    print(image_path)
    image = Image(image_path)
    image.width = 100
    image.height = 100
    # TODO 기준 파일에 맞춰 정보 수정 필요(이미지 삽입 열 지정)
    wb2Sheet.add_image(image, anchor='X'+str(i))
  except:
    pass
...
# TODO 기준 파일에 맞춰 정보 수정 필요
ws = wb.Worksheets('2024_여름')

# TODO 기준 파일에 맞춰 정보 수정 필요
ws.Range('A:X').Sort(Key1=ws.Range('W1'), Order1=2)
